...
[<Fact>]
let ``Queen with a valid position``() = create (2, 2) |> should equal true

[<Fact>]
let ``Queen must have positive row``() = create (-2, 2) |> should equal false

[<Fact>]
let ``Queen must have row on board``() = create (8, 4) |> should equal false

[<Fact>]
let ``Queen must have positive column``() = create (2, -2) |> should equal false

[<Fact>]
let ``Queen must have column on board``() = create (4, 8) |> should equal false
...
[<Fact>]
let ``Queens can not attack if on same square``() =
    let whiteQueen = (4, 4)
    let blackQueen = (4, 4)
    canAttack blackQueen whiteQueen |> should equal false
